...
# -----------------------------------------------------------
# 3. [4번 기능] 30초 자동 새로고침 플러그인 (st.fragment 활용)
# -----------------------------------------------------------
@st.fragment(run_every="30s")
def render_dashboard():
    # 데이터 로드 안내 (작은 글씨로 우측 상단이나 하단에 표시되도록 설정)
    ...
    if len(raw_data) > 0:
        ...
        # [3번 기능] 아이콘 레이어를 위한 설정 추가
        # 정상 비행기는 노란색 아이콘, 위험 비행기는 빨간색 아이콘 지정
        def assign_icon_data(status):
            if status == '위험(급강하)':
                # 빨간색 비행기 아이콘 매핑 (오픈소스 이미지 주소 활용)
                return {"url": "https://img.icons8.com/isometric/100/fa3131/airplane-mode-on.png", "width": 128, "height": 128, "anchorY": 64}
            # 노란색 비행기 아이콘 매핑
            return {"url": "https://img.icons8.com/isometric/100/fab005/airplane-mode-on.png", "width": 128, "height": 128, "anchorY": 64}

        df['icon_data'] = df['status'].apply(assign_icon_data)
        ...
        layer = pdk.Layer(
            ...
            get_size=45,                 # 아이콘 크기
            pickable=True,
            get_angle="-true_track",     # 항공기 진행 방향 각도만큼 아이콘 회전 (Pydeck은 시계반대방향이 기본이라 마이너스 부호 적용)
        )
